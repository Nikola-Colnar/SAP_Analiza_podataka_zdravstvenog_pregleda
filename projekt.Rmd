...
```{r, include=FALSE}
knitr::opts_chunk$set(fig.width = 6, fig.height = 4, out.width = "100%", fig.align = "center")
...
for (bmi_cat in bmi_categories) {
  ...
  hist_combined <- hist_hi + hist_lo + 
    plot_layout(ncol = 2) + 
    plot_annotation(title = paste("Histograme krvnog tlaka - BMI Kategorija:",
                                  bmi_cat)) &
...
plot10 <- ggplot(data.frame(x = fit.ap_lo$fitted.values,
  ...
  labs(title = "Reziduali u odnosu na fitane vrijednosti (ap_lo)",
       x = "Fitane vrijednosti", y = "Reziduali") +
  theme_minimal()
combined_plot <- (plot6 + plot7) / (plot8 + plot10)
print(combined_plot)
