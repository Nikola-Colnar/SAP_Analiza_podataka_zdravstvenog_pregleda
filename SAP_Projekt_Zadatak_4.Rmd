...
Sada metodom najmanjih kvadrata pokušavamo uspostaviti vezu između BMI-a i krvnog tlaka. Na sljedećem grafu možemo vidjeti graph raspršenja između sistoličkog tlaka i BMI-a kao i pravac linearne regresije koji smo izračunali iz podataka. Pokušavali smo linearna regresiju s polinomima viših stupnjeva, ali su svi stupnjevi bili veoma slični pravcima i nisu poboljšavali vrijednost $R^2$. Zbog toga smo dali prednost najjednostavnijem modelu, a to je naravno pravac.
```{r}
...
summary(fit.ap_hi)
```
Vidimo blagi pozitivan trend, ali $R^2$ vrijednost je 0.0564 ($R^2_{adj}$ je  0.05638) što ukazuje na loš fit modela, no mi ćemo svakako sada nastaviti s analizom reziduala.
```{r res}
...
```
Reziduali padaju na testu normalnosti (i KS i Lilliefors), a to nas baš i ne čudi ako pogledamo histogram reziduala na kojem se ne vide naznake normalnosti. 
Sada možemo zaključiti da je nemoguće predvidjeti sistolički krvni tlak iz BMI-a.


Za dijastolički krvni tlak ponavljamo isti postupak. I zadržat ćemo model pravca iz istog raloga kao i za sistolički tlak.
```{r}
...
Vidimo blagi pozitivan trend, ali vidimo (iz p vrijednosti) da regresor ima jako malenu značajnost. Također, $R^2$ vrijednost je sada 0.04008 ($R^2_{adj}$ je  0.04007) što opet ukazuje na loš fit modela, no mi ćemo svakako opet nastaviti s analizom reziduala.
Analiza reziduala
```{r}
...
print(lilliefors_test)
```
Reziduali se barem grafički ponašaju ljepše nego u slučaju sistoličkog krvnog tlaka, ali ne dovoljno da prođu test normalnosti. Nemoguće je (na temelju ovih podataka) predvidjeti dijastolički krvni tlak iz BMI-a.
...
S obzirom da je tjelesna aktivnost binarna kategorijska varijabla nije loša ideja staviti ju u model višestruke regresije, ali 

```{r}
#fit.multi = lm(ap_hi ~ BMI + age + active + smoke + AgeinYr + gender + alco + cardio, filtered_data)
